<x-app-layout>
    <x-slot name="header">
        <div class="flex" dir="rtl">
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                <a href="{{ route('position.create') }}" class="scale-100 p-6 bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500" >
                    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight pull-right" >
                        {{ __('++') }}
                    </h2>
                </a>
                            ...
                            <tbody>
                                @foreach($positions  as $index => $position)
                                    <tr class="bg-secondary">
                                        ...
                                        <td scope="row" class="max-w-3xl mx-auto sm:px-6 lg:px-4" >{{ $position->position }}</td>
                                        <td class="">
                                            <a href="{{ route('country.edit',['id'=>$position->id]) }}">
